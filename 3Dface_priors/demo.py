...
def main():
	...
	#with tf.Graph().as_default() as graph,tf.device('/device:GPU:0'):
	with tf.Graph().as_default() as graph, tf.device('/cpu:0'):
		...
		#images = tf.placeholder(name='input_imgs', shape=[None, 200, 200, 3], dtype=tf.float32)
		...
		with tf.Session() as sess:
			for filename in input_pathss:
				save_path = filename.replace('dataset', 'training_set')
				if not os.path.exists(save_path):
					os.makedirs(save_path)
